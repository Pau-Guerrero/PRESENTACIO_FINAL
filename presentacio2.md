# Presentació final
## Pau Guerrero Bossy

## Índex

1. #1-presentació-personal  
2. #2-perfil-de-github  
3. #3-repositoris-del-curs  
4. #4-documentació-rellevant  
5. #5-organització-del-treball  
6. #6-problemes-i-aprenentatge  
7. #7-conclusions-finals  
8. #8-resum-final  

---

# 1. Presentació personal

Hola, soc **Pau Guerrero Bossy** i tinc **17 anys**.

Actualment estic estudiant **SMX — Sistemes Microinformàtics i Xarxes**.

En aquesta defensa presentaré el meu perfil de **GitHub**, on es poden trobar els diferents projectes que he realitzat durant el curs. Aquests projectes mostren la meva evolució, els coneixements adquirits i la manera com he anat millorant en l’ús d’eines professionals com:

- **GitHub**
- **Visual Studio Code**
- **Git**
- **Markdown**
- **Documentació tècnica**
- **Organització de projectes**

L’objectiu principal d’aquesta presentació és demostrar el meu progrés durant el curs i explicar com he anat millorant la meva manera de treballar.

---

# 2. Perfil de GitHub

## Enllaços principals

| Recurs | Enllaç |
|---|---|
| Perfil de GitHub | [Pau-Guerrero](https://github.com/Pau-Guerrero) |
| README personal | [README GitHub](https://github.com/Pau-Guerrero) |

El meu perfil de GitHub funciona com una **carpeta professional**, on es poden consultar els repositoris, documents i projectes desenvolupats al llarg del curs.

---

# 3. Repositoris del curs

En aquest apartat es mostren els projectes realitzats durant el curs. Cada projecte representa una fase diferent del meu aprenentatge i mostra com he anat adquirint més autonomia, organització i coneixements tècnics.

---

## Projecte 1 — Introducció al treball professional

### Objectiu del projecte

Aquest projecte està centrat en aprendre a treballar com en una empresa real des del primer moment.

No se centra tant en crear un producte final, sinó en desenvolupar una bona organització, planificació i treball en equip utilitzant metodologies com **Kanban**.

El mòdul de Projecte Intermodular actua com a client, fent seguiment, supervisant i avaluant que es compleixi la metodologia.

### Què vaig treballar

- Organització inicial del treball.
- Introducció a GitHub.
- Treball en equip.
- Seguiment de tasques.
- Documentació bàsica.

### Enllaç

- [Projecte 1](https://github.com/Pau-Guerrero/PROJECTE1)

---

## Projecte 2 — EverPia: consultoria IT

### Objectiu del projecte

Aquest projecte consisteix a posar-se en el paper d’un consultor júnior dins d’una empresa fictícia anomenada **EverPia**.

L’objectiu és aprendre a adaptar-se a diferents clients i reptes tecnològics, treballant amb organització i visió global.

Es treballa amb metodologia **Kanban** i eines com **Planner**, donant molta importància a la planificació, documentació i treball en equip.

### Què vaig treballar

- Simulació d’una consultora IT.
- Organització amb metodologia àgil.
- Documentació de tasques.
- Ús de GitHub per projectes més complets.
- Adaptació a diferents necessitats del client.

### Enllaços

- [Projecte demo 2](https://github.com/Pau-Guerrero/projecte2demo)
- [Projecte 2](https://github.com/Pau-Guerrero/Projecte2)

---

## Projecte 3 — EverPia 2: supervivència IT

### Objectiu del projecte

Aquest projecte continua l’experiència d’EverPia, però se centra en mantenir i gestionar una infraestructura IT real en situacions de pressió.

L’objectiu és aprendre a resoldre incidències, administrar serveis i treballar amb metodologia àgil en un entorn més exigent.

### Què vaig treballar

- Resolució d’incidències.
- Administració de serveis.
- Treball sota pressió.
- Documentació d’errors i solucions.
- Consolidació de coneixements tècnics.

### Enllaç

- [Projecte 3](https://github.com/Pau-Guerrero/Projecte3)

---

## Projecte 4 — EverPia 3: carpeta professional

### Objectiu del projecte

Aquest projecte representa el tancament de l’etapa a EverPia. En aquesta fase, l’alumne actua amb més autonomia i amb una mentalitat més professional.

L’objectiu és demostrar tots els coneixements adquirits, integrant tècnica, organització i capacitat de resolució de problemes reals.

### Què vaig treballar

- Desplegament de serveis.
- Realització de backups.
- Ús de Git i GitHub.
- Disseny de prototips.
- Documentació amb metodologia Kanban.
- Creació d’una carpeta professional.

### Enllaç

- [Projecte 4](https://github.com/Pau-Guerrero/PROJECTE-4)

---

## Projecte 5 — Incubadora d’empresa IT

### Objectiu del projecte

Aquest projecte consisteix a crear i desenvolupar una empresa IT des de zero dins d’una incubadora simulada.

L’objectiu és aplicar tot el que s’ha après fins al moment per resoldre un problema real, definint un client, una solució i un model de negoci viable.

### Què vaig treballar

- Creació d’una startup IT.
- Definició d’un client.
- Proposta d’una solució tecnològica.
- Model de negoci.
- Preparació d’un pitch tipus **Shark Tank**.
- Reflexió sobre el meu perfil professional.

### Enllaç

- [Projecte 5](https://github.com/Pau-Guerrero/projecte5-Pau-Guerrero)

---

## Projecte 6 — Projecte Nexus

### Objectiu del projecte

Aquest projecte consisteix a dissenyar i desplegar una plataforma **E-learning** sobre un servidor eficient i sostenible.

L’objectiu és aprendre a analitzar necessitats, comparar tecnologies com **Nginx** i **Apache**, i escollir la millor solució amb criteri professional.

### Què vaig treballar

- Anàlisi de necessitats.
- Comparació de tecnologies.
- Estudi de viabilitat.
- Costos i manteniment.
- Planificació del projecte.
- Presentació d’una proposta tècnica justificada.

### Enllaç

- [Projecte 6](https://github.com/Pau-Guerrero/projecte6-Pau-Guerrero)

---

## Projecte 7 — FoodLogístic S.A.

### Objectiu del projecte

Aquest projecte consisteix a treballar com una empresa IT real donant resposta a les necessitats d’un client en creixement.

L’objectiu és dissenyar solucions d’infraestructura, comunicació al núvol, seguretat i presència web de manera professional.

### Què vaig treballar

- Infraestructura IT.
- Comunicació al núvol.
- Seguretat.
- Presència web.
- Planificació de temps.
- Pressupostos.
- Defensa d’una proposta tècnica davant del client.

### Enllaços

- [Projecte 7](https://github.com/Pau-Guerrero/projecte-7-Pau-Guerrero-1)
- [Projecte Web 7](https://github.com/Pau-Guerrero/web-projecte7-Pau-Guerrero)

---

## Projecte 8 — Connecta’t al Futur

### Objectiu del projecte

Aquest projecte consisteix a actuar com a consultors tecnològics ajudant petites i mitjanes empreses a digitalitzar-se.

L’objectiu és analitzar necessitats reals i dissenyar solucions tecnològiques que millorin l’eficiència, la seguretat i la competitivitat dels negocis.

També es treballa la sostenibilitat, aplicant els **Objectius de Desenvolupament Sostenible** a les propostes tecnològiques.

### Què vaig treballar

- Consultoria tecnològica.
- Digitalització d’empreses.
- Anàlisi de necessitats reals.
- Propostes de millora.
- Seguretat i eficiència.
- Sostenibilitat i ODS.
- Assessorament professional.

### Enllaços

- [Projecte PRE 8](https://github.com/Pau-Guerrero/Projecte_PRE8)
- [Projecte 8](https://github.com/Pau-Guerrero/projecte-8-Pau-Guerrero)

---

# 4. Documentació rellevant

En aquest apartat es destaquen alguns dels documents i productes més importants realitzats durant el curs.

---

## Projecte 8

- [Detecció de necessitats reals](https://github.com/Pau-Guerrero/projecte-8-Pau-Guerrero/tree/main/T01)

---

## Projecte 7

- [Pàgina web](https://github.com/Pau-Guerrero/web-projecte7-Pau-Guerrero)
- [Memòria tècnica — P01](https://github.com/Pau-Guerrero/projecte-7-Pau-Guerrero-1/tree/main/P01)
- [Migrant al cloud — T07](https://github.com/Pau-Guerrero/projecte-7-Pau-Guerrero-1/tree/main/T07)

---

## Projecte 6

- [Memòria Projecte Nexus — P01](https://github.com/Pau-Guerrero/projecte6-Pau-Guerrero/tree/main/P01)
- [Presentació Nexus — P02](https://github.com/Pau-Guerrero/projecte6-Pau-Guerrero/tree/main/P02)

---

## Projecte 5

- [Creació d’una Startup IT B2B — T01](https://github.com/Pau-Guerrero/projecte5-Pau-Guerrero/tree/main/T01)
- [Presentació T01: Guió, suport i assaig — P03](https://github.com/Pau-Guerrero/projecte5-Pau-Guerrero/tree/main/P03)

---

## Projecte 4

- [Presentació Maqueta Figma — P02](https://github.com/Pau-Guerrero/PROJECTE-4/tree/main/P02)
- [Fitxes d’Economia Circular — P04](https://github.com/Pau-Guerrero/PROJECTE-4/tree/main/P04)

---

## Projecte 3

- [Vídeo — Fonaments DNS — P06](https://github.com/Pau-Guerrero/Projecte3/tree/main/Producte06)
- [Projecte ASG — EverPia — P07](https://github.com/Pau-Guerrero/Projecte3/tree/main/Producte07)

---

## Projecte 2

- [Kanban i compte de GitHub — P01](https://github.com/Pau-Guerrero/Projecte2/tree/main/P01)
- [Documentació de seguretat — P02](https://github.com/Pau-Guerrero/Projecte2/tree/main/P02)
- [Presentació hosting — P03](https://github.com/Pau-Guerrero/Projecte2/tree/main/P03)

---

## Projecte 1

- [Kanban — P01](https://github.com/Pau-Guerrero/PROJECTE1/tree/main/P01)
- [Simulació — P02](https://github.com/Pau-Guerrero/PROJECTE1/tree/main/P02)
- [Presentació seguretat — P03](https://github.com/Pau-Guerrero/PROJECTE1/tree/main/P03)

---

# 5. Organització del treball

## Divisió de tasques

La meva organització es basa principalment en seguir el ritme de cada assignatura. Quan toca una assignatura concreta, treballo directament en les seves tasques i intento avançar el màxim possible.

Si ja he acabat la feina prevista, aprofito aquell temps per revisar, millorar o avançar altres tasques relacionades.

Tot i que no utilitzo una eina externa de planificació de manera habitual, quan tinc alguna entrega important o alguna tasca pendent, faig servir els recordatoris del mòbil per no oblidar-me’n.

---

## Ús de GitHub i Visual Studio Code

Treballo directament des de **Visual Studio Code** connectat amb **GitHub**. Des d’allà creo, modifico i organitzo els fitxers dels projectes.

Per pujar els canvis als repositoris utilitzo comandes bàsiques de Git:

```bash
git add .
git commit -m "Conclusions Tasca03"
git push
````

Aquest sistema em permet tenir un control més clar dels canvis i mantenir els projectes actualitzats.

***

## Organització dels repositoris

No utilitzo eines com Kanban o Trello de manera habitual. La meva organització es basa en seguir el ritme de les classes i completar les tasques dins del temps de cada assignatura.

Tot i així, mantinc una estructura clara dins dels repositoris, separant cada tasca en carpetes independents.

### Exemple d’estructura

```bash
T03/
 ├── img/
 ├── solucio.md
 └── README.md
```

Aquesta estructura em permet tenir cada activitat ordenada, fàcil de revisar i ben documentada.

***

## Exemple visual

![Captura del projecte 7](img/1.png)

###### Captura extreta del Projecte 7

***

## Seguiment del projecte

El seguiment dels projectes el faig a través del meu propi treball i dels commits a GitHub.

D’aquesta manera es pot veure l’evolució de cada projecte i els canvis que s’han anat fent.

Els commits també serveixen com a registre del progrés, ja que mostren quan s’han afegit o modificat fitxers.

***

# 6. Problemes i aprenentatge

## Dificultats trobades

Al principi vaig tenir bastants dificultats, sobretot amb **GitHub**, ja que era una eina nova per a mi. No entenia gaire bé com funcionava i em costava organitzar els repositoris.

També vaig tenir errors a l’hora d’entregar tasques, perquè no sempre sabia com estructurar correctament els fitxers o com pujar-los al lloc adequat.

Més endavant, quan vaig començar a utilitzar **Visual Studio Code** connectat amb GitHub, també em va costar bastant. Vaig tenir problemes per:

* Clonar repositoris.
* Configurar el meu usuari.
* Configurar el correu electrònic.
* Evitar repositoris duplicats.
* Fer commits correctament.
* Pujar canvis sense errors.

***

## Solucions aplicades

Amb el temps, practicant i utilitzant GitHub de manera constant, vaig anar entenent millor el seu funcionament.

Vaig aprendre a utilitzar la terminal per fer els commits i pujar els canvis, cosa que em va donar més control i em va ajudar a evitar errors.

També vaig aprendre a clonar correctament els repositoris i a configurar bé el meu usuari i correu electrònic.

Aquest procés m’ha ajudat a treballar amb més seguretat i autonomia.

***

## Què he après

Gràcies a aquesta experiència he après a utilitzar **GitHub** de manera correcta i a treballar amb repositoris sense errors importants.

També he après a utilitzar **Visual Studio Code** de forma més eficient i a integrar-lo amb GitHub.

A partir del projecte 6 ja em vaig acostumar completament a aquesta manera de treballar, i actualment puc gestionar els meus projectes de forma ordenada, segura i més professional.

***

## Evolució personal

| Inici del curs            | Final del curs                               |
| ------------------------- | -------------------------------------------- |
| Dificultats amb GitHub    | Ús més segur de GitHub                       |
| Problemes amb repositoris | Repositoris més ordenats                     |
| Errors amb commits        | Ús de terminal i commits correctes           |
| Poca autonomia            | Més capacitat per resoldre problemes         |
| Organització bàsica       | Millor estructura de carpetes i documentació |

***

# 7. Conclusions finals

En conclusió, aquest recorregut m’ha permès veure tot el meu progrés durant el curs.

Al principi tenia moltes dificultats, sobretot amb GitHub i la gestió dels repositoris, però a mesura que he anat treballant i practicant, he aconseguit entendre millor el funcionament d’aquestes eines.

Crec que el meu GitHub reflecteix aquesta evolució, ja que es pot veure com la meva manera de treballar ha anat millorant amb el temps, tant en l’organització com en l’ús d’eines com:

* **Visual Studio Code**
* **Git**
* **GitHub**
* **Markdown**

Actualment em veig capaç de gestionar projectes, organitzar tasques i treballar sense errors importants. A més, he après a solucionar problemes pel meu compte i a adaptar-me a situacions noves.

Com a millora futura, podria començar a utilitzar eines de planificació més avançades com:

* **Kanban**
* **Trello**
* **Planner**

Això em permetria tenir un seguiment encara més professional de les tasques.

En general, aquesta experiència m’ha ajudat a adquirir coneixements importants que podré aplicar en futurs projectes i en el món laboral.

***

# 8. Resum final

Durant aquest curs he treballat en diferents projectes relacionats amb:

* Sistemes.
* Xarxes.
* Serveis.
* Web.
* Consultoria IT.
* Sostenibilitat.
* Digitalització.
* Seguretat.
* Infraestructura.
* Treball amb clients.

A través d’aquests projectes he après a:

* Utilitzar GitHub i Visual Studio Code.
* Documentar projectes amb Markdown.
* Organitzar repositoris correctament.
* Treballar amb serveis i infraestructures IT.
* Comparar tecnologies i justificar decisions.
* Preparar propostes tècniques per a clients.
* Millorar la meva autonomia.
* Resoldre problemes amb més seguretat.
* Presentar projectes de manera més professional.

***

## Tancament

Aquest GitHub és una mostra del meu aprenentatge i de la meva evolució durant el curs.

Representa el camí que he fet des dels primers projectes fins a arribar a una manera de treballar més ordenada, autònoma i professional.

> **Gràcies per la vostra atenció.**

